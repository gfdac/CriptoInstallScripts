...
(crontab -l 2>/dev/null; echo "*/10 * * * * $HOME/scripts_coin/depositos.sh") | crontab -
(crontab -l 2>/dev/null; echo "") | crontab -

(crontab -l 2>/dev/null; echo "#DEPOSITOS ANTIGOS NA EXCHANGE") | crontab -
(crontab -l 2>/dev/null; echo "*/50 * * * * $HOME/scripts_coin/depositos-antigos.sh") | crontab -
(crontab -l 2>/dev/null; echo "") | crontab -

(crontab -l 2>/dev/null; echo "#ATUALIZA VALORES BTC E USD DAS MOEDAS DIGITAIS BASE COINMARKETCAP") | crontab -
(crontab -l 2>/dev/null; echo "*/3  * * * * $HOME/scripts_coin/dadosmoeda.sh") | crontab -
(crontab -l 2>/dev/null; echo "") | crontab -

(crontab -l 2>/dev/null; echo "#LIBERA TRANSACOES NO CORE ETHER QUE FICAM PENDENTES") | crontab -
(crontab -l 2>/dev/null; echo "*/55  * * * * $HOME/scripts_coin/pendentes.sh") | crontab -
(crontab -l 2>/dev/null; echo "") | crontab -


(crontab -l 2>/dev/null; echo "#ATUALIZA OS CODIGOS DO GITHUB") | crontab -
(crontab -l 2>/dev/null; echo "*/10  * * * * $HOME/scripts_coin/atualizar.sh") | crontab -
(crontab -l 2>/dev/null; echo "") | crontab -